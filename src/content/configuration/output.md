...
T> Can be given an `object` since webpack 3.1.0. <span class='important'>Effective for `libraryTarget: 'umd'`</span>.
...
Configure <span class='definition'>how the library will be exposed</span>. Any one of the following options can be used. Please note that this option works in conjunction with the value assigned to [`output.library`](#outputlibrary). For the following examples, it is assumed that this value is configured as `MyLibrary`.
